...
- **Phase 0 probe — RULED 2026-08-13: STEWARD-CLIENT MODEL.** Server-side `CreatePed` was probed live (`/sr_probe_spawn`, test server, `onesync=on`): the native exists and returns a handle, but the entity never materialises (`no entity exists after 5s`). With OneSync on, that is a RedM verdict, not environment. Therefore: the first on-duty member at the ranch becomes **steward**, spawns peds **networked** (dressed at spawn — the standing rule below), registers netIds with the server's spawn ledger; on steward disconnect the server re-assigns and the new steward re-resolves entities by netId (respawn fallback). The server remains authoritative over what exists and where; clients only materialise it.
...
- **Every spawned ped MUST be dressed [Wilbur ruling 2026-08-13]:** a fresh RDR3 ped carries no outfit variation and can render **invisible**. The proven pattern (banking teller → realestate receptionist lineage) is the client-side two-native dress — `_SET_RANDOM_OUTFIT_VARIATION` `0x283978A15512B2FE` then `_UPDATE_PED_VARIATION` `0xCC8CA3E88256E58F` — with a ~100ms settle beat, applied twice. Both verified in natives_rdr3.json (the latter's own description: "needed after first creation"). Server-spawned entities get dressed by the nearest/steward client (the Phase 0 probe already does this by netId); client-spawned ones dress at spawn.
...
1. **Server-side CreatePed on RedM/OneSync** — ~~Phase 0 probe decides~~ **RULED 2026-08-13: not viable (probed live, onesync=on, entity never materialises) → steward-client model** (§6.3).
